import {tagRegBuilder} from '../src/util/tag-reg-builder';
import {DylanTemplateEngine, keyWordsReg} from '../src/dylan-template-engine';

describe('DylanTemplateEngine', () => {

  beforeEach(() => {
  });

  it('simple template engine', () => {
    const templateEngine = new DylanTemplateEngine();
  ...
  it('use for each success and compress success', () => {
    const templateEngine = new DylanTemplateEngine({compress: true});
    const template = `
    <ul>
